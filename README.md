...
当番茄钟运行或暂停时，屏幕会显示独立番茄钟页：

- 第一行：大号倒计时，例如 `24:59`
- 第二行：当前阶段 `FOCUS` 或 `BREAK`
- 第三行：当前状态 `Running` 或 `Paused`

当番茄钟运行时：

- 普通待机时钟页会暂时隐藏
- 如果被语音唤醒进入对话，仍会临时切到眼睛页
- 对话结束后会重新回到番茄钟页
...
设备网页配置页支持直接设置番茄钟时长并控制番茄钟：

- 可配置专注时长，例如 `25`、`40`、`50`
- 可配置休息时长，例如 `5`、`10`、`3`
- 支持网页按钮直接 `开始 / 暂停 / 继续 / 取消`
